...
class BridgeLegacy extends Extension {
    ...
    rename(topic, message) {
        ...
        this.commands.$rename({id: json.old, newName: json.new});
    }
    ...
    renameLast(topic, message) {
        ...
        this.commands.$rename({id: this.lastJoinedDeviceName, newName: message});
    }
    ...
    addGroup(topic, message) {
        ...
        this.commands.$group.$add({name, id});
    }

    removeGroup(topic, message) {
        this.commands.$group.$remove({id: message});
    }

    async forceRemove(topic, message) {
        await this.commands.$device.$remove({id: message, action: 'force_remove'});
    }

    async remove(topic, message) {
        await this.commands.$device.$remove({id: message, action: 'remove'});
    }

    async ban(topic, message) {
        await this.commands.$device.$remove({id: message, action: 'ban'});
    }
