...
class Groups extends Extension {
    ...
    async onMQTTMessage(topic, message) {
        ...
        if (type === 'add') {
            await this.api.groups.addDevice(resolvedEntityGroup, resolvedEntityDevice);
        } else if (type === 'remove') {
            await this.api.groups.removeDevice(resolvedEntityGroup, resolvedEntityDevice);
        } else { // remove_all
            ...
            for (const settingsGroup of settings.getGroups()) {
                const resolvedEntityGroup = this.zigbee.resolveEntity(settingsGroup.ID);
                await this.api.groups.removeDevice(resolvedEntityGroup, resolvedEntityDevice, true);
            }
        }
    }
}
