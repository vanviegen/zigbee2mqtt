...
class Groups extends Extension {
    ...
    async onMQTTMessage(topic, message) {
        ...
        if (type === 'add') {
            this.commands.$group.$addDevice({groupId: resolvedEntityGroup.settings.ID, deviceId: resolvedEntityDevice.device.ieeeAddr});
            ...
            this.eventBus.emit('groupDeviceAdded', {group: resolvedEntityGroup.group, device: resolvedEntityDevice.device});
            ...
            this.eventBus.emit('groupDeviceRemoved', {group: resolvedEntityGroup.group, device: resolvedEntityDevice.device});
            ...
            for (const settingsGroup of settings.getGroups()) {
                ...
                const group = this.zigbee.getGroup(settingsGroup.ID);
                this.eventBus.emit('groupDeviceRemoved', {group, device: resolvedEntityDevice.device});
