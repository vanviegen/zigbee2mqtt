...
const ExtensionWebSocket = require('./extension/webSocket');
const ExtensionTCP = require('./extension/tcp');
const ExtensionHTTP = require('./extension/http');
const ExtensionNewMQTT = require('./extension/newMqtt');

const API = require('./extension/api');
const DevicesAPI = require('./extension/devicesApi');
const GroupsAPI = require('./extension/groupsApi');
...
class Controller {
    constructor() {
        ...
        if (settings.get().websocket.enabled) {
            this.extensions.push(new ExtensionWebSocket(...args));
        }

        if (settings.get().tcp.enabled) {
            this.extensions.push(new ExtensionTCP(...args));
        }

        if (settings.get().http.enabled) {
            this.extensions.push(new ExtensionHTTP(...args));
        }

        if (settings.get().mqtt.new_api_base_topic) {
            this.extensions.push(new ExtensionNewMQTT(...args));
        }
    ...
    async publishEntityState(IDorName, payload, stateChangeReason=null) {
        ...
        this.eventBus.emit('publishEntityState', {
