    ...
    properties: {
        ...
        mqtt: {
            ...
            properties: {
                ...
                new_api_base_topic: {type: 'string'},
                server: {type: 'string'},
...
function whitelistDevice(ID, remove) {
    const settings = get();
    ...
    if (settings.whitelist.includes(ID)) {
        if (remove) {
            settings.whitelist.splice(settings.whitelist.indexOf(ID), 1);
        } else {
            throw new Error(`Device '${ID}' already whitelisted`);
        }
    } else {
        if (remove) {
            throw new Error(`Device '${ID}' is not whitelisted`);
        } else {
