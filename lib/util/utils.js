...
async function setProperties(object, prefix, args, properties) {
    for (const name of Object.keys(properties)) {
        const methodName = (prefix + name).replace(/_(.)/g, (all, char) => char.toUpperCase());
        if (typeof object[methodName]==='function') {
            await object[methodName](...args, properties[name], properties);
            delete properties[name];
        }
    }

    if (Object.keys(properties).length) {
        throw new Error(`Unhandled set keys: ${Object.keys(properties).join(', ')}`);
    }
}
